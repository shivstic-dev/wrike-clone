...
describe('dependency paths', () => {
  it('uses the stable task-edge anchors for every dependency type', () => {
    ...
    expect(dependencyAnchors(DependencyType.START_TO_START, predecessor, dependent).fromX).toBe(
      predecessor.left,
    );
    expect(dependencyAnchors(DependencyType.FINISH_TO_FINISH, predecessor, dependent).toX).toBe(
      dependent.right,
    );
  });

  it('creates visible elbows for backward and same-row links without moving anchors', () => {
    const backward = dependencyPath(
      DependencyType.FINISH_TO_START,
      { left: 180, right: 240, y: 22 },
      { left: 100, right: 150, y: 22 },
    );
