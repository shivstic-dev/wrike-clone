    ...
    return (
      ...
      >
        ...
        <div className="mt-1 space-y-0.5">
          ...
          {dayTasks.length > 3 && (
            <span className="text-[11px] text-slate-400">+{dayTasks.length - 3} more</span>
          )}
        </div>
      </div>
    );
      ...
      <div>
        <div className="grid grid-cols-7 border-b border-slate-200">
          {dayHeaders.map((d) => (
            <div
              key={d}
              className="bg-slate-50 px-2 py-2 text-center text-xs font-semibold uppercase text-slate-500"
            >
              {d}
            </div>
          ))}
        </div>
        <div className="grid grid-cols-7">{days.map((day) => renderDay(day))}</div>
      </div>
    );
  };
    ...
    return (
      <div>
        ...
        <div className="divide-y divide-slate-100">
          {dayTasks.length === 0 ? (
            <p className="p-8 text-center text-sm text-slate-400">No tasks due on this day.</p>
          ) : (
            dayTasks.map((task) => (
              <Link
                key={task.id}
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white">
      ...
      <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3">
        ...
        <div className="flex items-center gap-2">
          <button onClick={navigateToday} className="btn-secondary btn-sm text-xs">
            Today
          </button>
