      ...
      <div className="w-full max-w-xl rounded-xl bg-white p-6 shadow-xl max-h-[80vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input className="input" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g., Notify on high priority" required />
          </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <button type="button" onClick={addCondition} className="btn-ghost btn-sm text-xs text-primary-600">
                + Add condition
              </button>
            </div>
            {conditions.map((cond, i) => (
              <div key={i} className="flex items-center gap-2 mb-2">
                <input className="input flex-1 text-sm" placeholder="Field" value={cond.field} onChange={(e) => updateCondition(i, 'field', e.target.value)} />
                ...
                <input className="input w-24 text-sm" placeholder="Value" value={cond.value} onChange={(e) => updateCondition(i, 'value', e.target.value)} />
                <button type="button" onClick={() => removeCondition(i)} className="p-1 text-red-400 hover:text-red-600">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <button type="button" onClick={addAction} className="btn-ghost btn-sm text-xs text-primary-600">
                + Add action
              </button>
            </div>
            {actions.map((action, i) => (
              <div key={i} className="flex items-center gap-2 mb-2">
                <select className="input flex-1 text-sm" value={action.type} onChange={(e) => updateAction(i, 'type', e.target.value)}>
                  {ACTION_TYPES.map((at) => (
                    <option key={at.value} value={at.value}>{at.label}</option>
                  ))}
                </select>
                <button type="button" onClick={() => removeAction(i)} className="p-1 text-red-400 hover:text-red-600">
                  <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                </button>
              </div>
            ))}
...
export function AutomationRules() {
  ...
  return (
    <div>
      ...
      {ruleList.length === 0 ? (
        <EmptyState title="No automation rules" description="Create rules to automate repetitive tasks." action={
          <button onClick={() => setShowCreateModal(true)} className="btn-primary btn-sm">Create your first rule</button>
        } />
      ) : (
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">
                    When {rule.triggerEvent.replace(/_/g, ' ')} → {rule.actions?.length || 0} action(s)
                  </p>
                </div>
              </div>
              <button
