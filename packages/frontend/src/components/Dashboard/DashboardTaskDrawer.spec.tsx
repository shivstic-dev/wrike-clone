...
describe('DashboardTaskDrawer', () => {
  it('renders the selected bucket task details and can be closed with Escape or Close', async () => {
    ...
    await act(async () => {
      ...
      root.render(<DashboardTaskDrawer bucket="ready_for_handoff" query={{ data: response, isLoading: false, error: null, refetch: vi.fn() }} onClose={onClose} />);
      await Promise.resolve();
    });

    expect(container.querySelector('[role="dialog"]')?.textContent).toContain('Ready for handoff');
    ['Send grant update', 'Grant campaign', 'Eli Assignee', 'Maya Owner', 'Aug'].forEach((text) => expect(container.textContent).toContain(text));
    act(() => document.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', bubbles: true })));
    expect(onClose).toHaveBeenCalledOnce();
    act(() => container.querySelector<HTMLButtonElement>('button[aria-label="Close task list"]')?.click());
  ...
  it('keeps loading, retry, and empty states safe inside the selected bucket', async () => {
    ...
    await act(async () => {
      root = createRoot(container);
      root.render(<DashboardTaskDrawer bucket="blocked" query={{ isLoading: true, error: null, refetch }} onClose={vi.fn()} />);
      await Promise.resolve();
    });
    expect(container.querySelector('[role="status"]')?.getAttribute('aria-label')).toBe('Loading blocked tasks');

    await act(async () => {
      root?.render(<DashboardTaskDrawer bucket="blocked" query={{ isLoading: false, error: new Error('offline'), refetch }} onClose={vi.fn()} />);
      await Promise.resolve();
    });
    expect(container.querySelector('[role="alert"]')?.textContent).toContain('Task list is unavailable');
    act(() => container.querySelector<HTMLButtonElement>('[role="alert"] button')?.click());
    expect(refetch).toHaveBeenCalledOnce();

    await act(async () => {
      root?.render(<DashboardTaskDrawer bucket="blocked" query={{ data: { ...response, bucket: 'blocked', data: [] }, isLoading: false, error: null, refetch }} onClose={vi.fn()} />);
      await Promise.resolve();
    });
