...
function mountMutation<TInput>(
  ...
  act(() => root.render(createElement(QueryClientProvider, { client }, createElement(() => {
    mutateAsync = hook().mutateAsync;
    return null;
  }))));
  return async (input: TInput) => {
    let result: unknown;
    await act(async () => { result = await mutateAsync?.(input); });
    return result;
  };
}

const query = { from: '2026-07-01', to: '2026-07-31', perPage: 50, includeCriticalPath: true };
const timeline: TimelineResponse = { tasks: [], unscheduled: [], dependencies: [], meta: { from: query.from, to: query.to, nextCursor: null } };
...
describe('timeline API', () => {
  it('normalizes dashboard queries and has canonical cache keys', async () => {
    ...
    await expect(requestTimeline({ kind: 'dashboard', departmentId: ' department-1 ' }, query)).resolves.toBe(timeline);
    const [, config] = vi.mocked(apiClient.get).mock.calls[0] ?? [];
    expect(apiClient.get).toHaveBeenCalledWith('/timeline', expect.any(Object));
    expect(config?.params?.toString()).toBe('from=2026-07-01&to=2026-07-31&departmentId=department-1&perPage=50&includeCriticalPath=true');
    expect(timelineKeys.scope({ kind: 'dashboard', departmentId: 'department-1' }, query)).toEqual([
      'timeline', 'dashboard', 'department-1', 'from=2026-07-01&to=2026-07-31&departmentId=department-1&perPage=50&includeCriticalPath=true',
    ]);
  });

  it('uses the project route and does not pass a projectId query parameter', async () => {
    vi.mocked(apiClient.get).mockResolvedValue({ data: timeline });
    await requestTimeline({ kind: 'project', projectId: 'project-1' }, { ...query, projectId: 'ignored' });
    const [, config] = vi.mocked(apiClient.get).mock.calls[0] ?? [];
  ...
  it('optimistically updates schedules and rolls back every matching timeline cache on error', async () => {
    ...
    const previous = { ...timeline, tasks: [{ id: 'task-1', startDate: '2026-07-02', dueDate: '2026-07-03' }] } as TimelineResponse;
    ...
    await expect(mutate({ taskId: 'task-1', startDate: '2026-07-04', dueDate: '2026-07-05', expectedUpdatedAt: 'v1' })).rejects.toThrow('offline');
    expect(apiClient.patch).toHaveBeenCalledWith('/tasks/task-1/schedule', { startDate: '2026-07-04', dueDate: '2026-07-05', expectedUpdatedAt: 'v1' });
  ...
  it('uses dependency create, update, and delete routes', async () => {
    ...
    expect(apiClient.patch).toHaveBeenCalledWith('/tasks/dependencies/dep-1', { dependencyType: DependencyType.START_TO_START, lagDays: 0 });
