...
export async function up(knex: Knex): Promise<void> {
  await knex.raw(handoffSchemaSql);
}

export async function down(knex: Knex): Promise<void> {
  await knex.raw(`
DROP INDEX IF EXISTS idx_tasks_tenant_handoff_ready;

ALTER TABLE tasks
  DROP CONSTRAINT IF EXISTS tasks_handoff_confirmation_check,
  DROP CONSTRAINT IF EXISTS tasks_handoff_status_check,
  DROP COLUMN IF EXISTS handoff_confirmed_at,
  DROP COLUMN IF EXISTS handoff_confirmed_by,
  DROP COLUMN IF EXISTS handoff_ready_at,
  DROP COLUMN IF EXISTS handoff_owner_id,
  DROP COLUMN IF EXISTS handoff_status,
  DROP COLUMN IF EXISTS handoff_required;
`);
}
