...
SELECT tenant_id, workspace_id, user_id FROM workspace_members WHERE role = 'dept_admin'
ON CONFLICT (department_id, user_id) DO NOTHING;
ALTER TABLE workspace_members DROP CONSTRAINT IF EXISTS workspace_members_role_check;
UPDATE workspace_members SET role = CASE WHEN role = 'dept_admin' THEN 'employee' WHEN role = 'member' THEN 'employee' ELSE role END;
ALTER TABLE workspace_members ADD CONSTRAINT workspace_members_role_check CHECK (role IN ('employee', 'manager'));
...
ALTER TABLE tasks ALTER COLUMN status TYPE task_status_department_v1 USING (CASE status::text WHEN 'done' THEN 'completed' WHEN 'cancelled' THEN 'blocked' WHEN 'backlog' THEN 'todo' WHEN 'in_review' THEN 'in_progress' ELSE status::text END)::task_status_department_v1;
...
ALTER TABLE tasks ALTER COLUMN priority TYPE task_priority_department_v1 USING (CASE priority::text WHEN 'none' THEN 'low' WHEN 'urgent' THEN 'critical' ELSE priority::text END)::task_priority_department_v1;
ALTER TABLE projects ALTER COLUMN priority TYPE task_priority_department_v1 USING (CASE priority::text WHEN 'none' THEN 'low' WHEN 'urgent' THEN 'critical' ELSE priority::text END)::task_priority_department_v1;
...
ALTER TABLE tasks ADD CONSTRAINT tasks_visibility_check CHECK (visibility IN ('department', 'global'));
ALTER TABLE projects ADD CONSTRAINT projects_visibility_check CHECK (visibility IN ('department', 'global'));
ALTER TABLE tasks ADD COLUMN department_id UUID REFERENCES workspaces(id) ON DELETE RESTRICT;
UPDATE tasks t SET department_id = f.workspace_id FROM projects p JOIN folders f ON f.id = p.folder_id WHERE t.project_id = p.id;
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_workspace_statuses_unique_name ON workspace_statuses(workspace_id, name);
...
FROM workspaces w CROSS JOIN (VALUES ('To Do', '#64748b', 'not_started', 0), ('In Progress', '#3b82f6', 'active', 1), ('Completed', '#22c55e', 'completed', 2), ('Blocked', '#ef4444', 'blocked', 3)) AS status(name, color, category, sort_order)
ON CONFLICT (workspace_id, name) DO NOTHING;
ALTER TABLE department_heads ENABLE ROW LEVEL SECURITY;
ALTER TABLE department_heads FORCE ROW LEVEL SECURITY;
CREATE POLICY tenant_isolation ON department_heads FOR ALL USING (tenant_id = current_tenant_id()) WITH CHECK (tenant_id = current_tenant_id());
ALTER TABLE notification_log ENABLE ROW LEVEL SECURITY;
ALTER TABLE notification_log FORCE ROW LEVEL SECURITY;
CREATE POLICY tenant_isolation ON notification_log FOR ALL USING (tenant_id = current_tenant_id()) WITH CHECK (tenant_id = current_tenant_id());
